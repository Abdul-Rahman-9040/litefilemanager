...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['username']) && isset($_SESSION['user_type'])) {
        ...
        $uploadDir = $_SERVER['DOCUMENT_ROOT'] . '/project/notesuploads/';
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadFile)) {
            ...
            $sql = "INSERT INTO uploaded_notes(user_name,scheme,uploaded_date,semester,subject,module,document_path) VALUES (?, ?, ?, ?, ?, ?,?)";
            $stmt = mysqli_prepare($conn, $sql);

            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "sssssss", $user_name, $scheme, $date, $semester, $subject, $module, $uploadFile);
                if (mysqli_stmt_execute($stmt)) {
                    mysqli_stmt_close($stmt);
                    echo '<script>alert("Notes uploaded successfully!"); window.location.href = "admin_dashboard.php";</script>';
                    exit;
                } else {
